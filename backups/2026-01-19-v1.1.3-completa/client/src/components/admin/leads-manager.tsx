...
export default function LeadsManager() {
  ...
  // Nuevos estados para filtrado y paginación
  ...
  // Actualizar leadNotes cuando se selecciona un lead para visualizar
  useEffect(() => {
    if (viewLead) {
      setLeadNotes(viewLead.notes || "");
    } else {
      setLeadNotes("");
    }
  }, [viewLead]);
    ...
    try {
      ...
      if (response) {
        ...
        const sanitizedLeads = response.map(lead => {
          ...
          // Crear una copia del lead
          const sanitizedLead: Lead = {
            ...lead,
            // Asegurarse de que los campos críticos existan y tengan valores válidos
            id: lead.id || 0,
            full_name: lead.full_name || lead.name || "",
            name: lead.name || lead.full_name || "",
            email: lead.email || "",
            investment_size: lead.investment_size || lead.investment || "",
            investment: lead.investment || lead.investment_size || "",
  ...
  const applyPaginationAndSort = () => {
    ...
    console.log("Muestra de lead para debug:", paginatedLeads[0]);
              ...
              {isLoading ? (
                ...
                displayedLeads.map((lead) => (
                  <TableRow key={lead.id}>
                    ...
                    <TableCell>{lead.full_name || lead.name || ""}</TableCell>
                    <TableCell>{lead.email}</TableCell>
                    <TableCell>{formatInvestmentRange(lead.investment_size || lead.investment || "")}</TableCell>
          ...
          {viewLead && (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-medium">
                      {new Date(viewLead.created_at).toLocaleString()}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <Badge variant={getStatusBadgeVariant(viewLead.status) as any} className="mt-1">
                      {viewLead.status === "nuevo" ? "Nuevo" : 
                       ...
                       viewLead.status === "inactivo" ? "Inactivo" :
                       // Compatibilidad con estados antiguos en inglés
                       viewLead.status === "new" ? "Nuevo" : 
                       viewLead.status === "contacted" ? "Contactado" :
                       viewLead.status === "qualified" ? "Calificado" :
                       viewLead.status === "converted" ? "Convertido" :
                       viewLead.status === "rejected" ? "Rechazado" : 
                       viewLead.status}
                    </Badge>
                  </div>
                </div>
              ...
              {viewLead.message && (
                <div className="mb-4">
                  <Label>Mensaje</Label>
                  <div className="mt-1 p-3 border rounded-md text-sm whitespace-pre-line">
                    {viewLead.message}
                  </div>
                </div>
              )}
              ...
              <div className="mb-4">
                <Label htmlFor="lead-notes">Notas (Solo visibles para administradores)</Label>
                <Textarea
                  id="lead-notes"
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mt-2">
                  <Button
                    size="sm"
                    variant={viewLead.status === "nuevo" ? "default" : "outline"}
                    onClick={() => updateLeadStatus(viewLead.id, "nuevo")}
                  ...
                  <Button
                    size="sm"
                    variant={viewLead.status === "contactado" ? "secondary" : "outline"}
                    onClick={() => updateLeadStatus(viewLead.id, "contactado")}
                    disabled={isUpdatingStatus}
                  >
                    Contactado
                  </Button>
                  <Button
                    size="sm"
                    variant={viewLead.status === "interesado" ? "default" : "outline"}
                    className={viewLead.status === "interesado" ? "bg-blue-500 hover:bg-blue-600 text-white" : ""}
                    onClick={() => updateLeadStatus(viewLead.id, "interesado")}
                    disabled={isUpdatingStatus}
                  >
                    Interesado
                  </Button>
                  <Button
                    size="sm"
                    variant={viewLead.status === "convertido" ? "default" : "outline"}
                    className={viewLead.status === "convertido" ? "bg-green-500 hover:bg-green-600 text-white" : ""}
                    onClick={() => updateLeadStatus(viewLead.id, "convertido")}
                    disabled={isUpdatingStatus}
                  >
                  ...
                  <Button
                    size="sm"
                    variant={viewLead.status === "no_interesado" ? "destructive" : "outline"}
                    onClick={() => updateLeadStatus(viewLead.id, "no_interesado")}
                    disabled={isUpdatingStatus}
                  >
