...
const PortfolioStats: React.FC = () => {
  ...
  const totalBenefits = React.useMemo(() => {
    return (totalFeesEarnedRaw || 0) + (totalCollectedFeesEarnedRaw || 0);
  }, [totalFeesEarnedRaw, totalCollectedFeesEarnedRaw]);
  ...
  const cumulativeEarningsData = React.useMemo(() => {
    ...
    const totalCollectedFees = parseFloat(totalCollectedFeesEarned.replace(/[^\d.]/g, ''));
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <Card className="flex flex-col h-full bg-gradient-to-b from-blue-950/90 to-indigo-950 dark:from-blue-950 dark:to-indigo-950 overflow-hidden border-0 shadow-lg">
        <CardContent className="p-6 flex flex-col h-full">
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <p className="text-slate-300 text-sm font-medium">{t('dashboard.totalValueFees', 'Valor Total + Comisiones')}</p>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="text-xs w-64">
                        Suma del valor inicial depositado más los fees actuales disponibles (no recolectados).
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
          ...
          {/* Gráfico Capital vs Beneficios */}
          <div className="mt-1 mb-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-slate-300">
                {t('dashboard.capitalVsBenefits', 'Capital vs Benefits')}
              </span>
              <span className="text-xs text-slate-400">
                {((totalBenefits / (totalCapital + totalBenefits)) * 100).toFixed(0)}% {t('dashboard.benefits', 'benefits')}
              </span>
            </div>
            <div className="h-32">
            ...
            <div className="flex justify-between items-center mt-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-slate-400">{t('dashboard.capital')}: {formatCurrency(totalCapital, true)}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-indigo-400"></div>
                <span className="text-xs text-slate-400">{t('dashboard.benefits')}: {formatCurrency(totalBenefits, true)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-auto">
            <Button className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white border-0 flex items-center justify-center gap-1.5 font-medium py-5 rounded-md">
              ...
              {dashboardTranslations[language]?.addLiquidity || "Añadir Liquidez"}
            </Button>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-6 flex flex-col h-full">
          ...
          <div className="flex justify-between items-start mb-3">
            <div>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <p className="text-slate-300 text-sm font-medium">{t('dashboard.totalEarnings', 'Ganancias Totales')}</p>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Info className="h-3.5 w-3.5 text-slate-400 cursor-help" />
                    ...
                    <TooltipContent>
                      <p className="text-xs w-64">
                        Valor total de los beneficios generados por todas tus posiciones.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="p-3 bg-blue-900/30 rounded-lg">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-xs font-medium text-slate-300">{t('dashboard.availableNow', 'Available Now')}</span>
              </div>
              {isLoadingPositions ? (
            ...
            <div className="p-3 bg-green-900/30 rounded-lg">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-xs font-medium text-slate-300">{t('dashboard.alreadyCollected', 'Already Collected')}</span>
              </div>
              {isLoadingPositions ? (
          ...
          <div className="p-3 bg-slate-800/40 rounded-lg mb-4">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-slate-300">{t('dashboard.last30Days', 'Last 30 Days')}</span>
              </div>
            ...
            {isLoadingPositions ? (
              ...
              <div className="text-xl font-bold text-white">
                {formatCurrency(currentMonthFees, false, '$')}
              </div>
            )}
          </div>
          ...
          <div className="mt-auto">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs font-medium text-slate-300">{dashboardTranslations[language]?.cumulativeEarnings || "Ganancias Acumuladas"}</span>
            </div>
            <div className="h-28">
      ...
      <Card className="flex flex-col h-full bg-gradient-to-b from-slate-950/90 to-purple-950 dark:from-slate-950 dark:to-purple-950 overflow-hidden border-0 shadow-lg">
        <CardContent className="p-6 flex flex-col h-full">
          ...
          <div className="flex justify-between items-start mb-3">
            <div>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <p className="text-slate-300 text-sm font-medium">{t('dashboard.activePositions', 'Posiciones Activas')}</p>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Info className="h-3.5 w-3.5 text-slate-400 cursor-help" />
                    ...
                    <TooltipContent>
                      <p className="text-xs w-64">
                        {t('dashboard.activePositionsTooltip', 'Current active positions generating yields.')}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs py-0.5 px-2 bg-purple-800/40 text-purple-300 rounded-md whitespace-nowrap">
                  {positionStats.poolsCount} {t('dashboard.pools', 'pools')}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-5 flex-grow">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-slate-300">{t('dashboard.roiTrend', 'ROI Trend')}</span>
                <Badge variant="secondary" className="px-1.5 py-0 h-auto text-xs rounded-sm bg-purple-900/40 text-purple-300 border-0">
                  {roiTrendData[roiTrendData.length - 1]?.roi.toFixed(2)}% {t('dashboard.current', 'current')}
                </Badge>
              </div>
              <div className="h-28">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-slate-300">{t('dashboard.poolPerformance', 'Pool Performance')}</span>
                <Badge variant="secondary" className="px-1.5 py-0 h-auto text-xs rounded-sm bg-purple-900/40 text-purple-300 border-0">
                  {positionStats.poolsCount} {t('dashboard.pools', 'pools')}
                </Badge>
              </div>
